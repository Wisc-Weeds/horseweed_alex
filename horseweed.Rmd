...
```{r model-function}
glmmTMB_model <- function(df) {
  glmmTMB(weed_control ~ trt + (1|rep), beta_family(link = "logit"), data = df)
}
```
...
horseweed2 <- horseweed1 %>% 
  mutate(model = map(data, glmmTMB_model))
```
...
```{r emmeans_function}
emmeans_function <- function(model) {
  emmeans(model, ~ trt, cont="pairwise", adjust="none", type="response", alpha=0.05)
}
```
